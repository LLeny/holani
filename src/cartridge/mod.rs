...
use crate::{alloc, bus, consts, mikey, suzy, TO_U16};
use alloc::{string::String, vec::Vec};
use bus::{Bus, BusStatus};
use cartridge_generic::{CartridgeGeneric, _1024KAUDIN_PINS, _128K_PINS, _256K_PINS, _512K_PINS};
use consts::{
    CART_PIN_A0, CART_PIN_A1, CART_PIN_A10, CART_PIN_A12, CART_PIN_A13, CART_PIN_A14, CART_PIN_A15,
    CART_PIN_A16, CART_PIN_A17, CART_PIN_A18, CART_PIN_A19, CART_PIN_A2, CART_PIN_A3, CART_PIN_A4,
    CART_PIN_A5, CART_PIN_A6, CART_PIN_A7, CART_PIN_A8, CART_PIN_A9, CART_PIN_AUDIN, CART_PIN_CE,
    CART_PIN_D0, CART_PIN_D1, CART_PIN_D2, CART_PIN_D3, CART_PIN_D4, CART_PIN_D5, CART_PIN_D6,
    CART_PIN_D7, CART_PIN_WE, CART_READ_TICKS, CART_WRITE_TICKS, SYSCTL1, SYSCTL1_POWER,
};
use eeprom::Eeprom;
...
const BLL_LOADER: [u8; 246] = [
    ,
...
fn write_data_pins(pins: u32, data: u8) -> u32 {
    write_pins(pins, u16::from(data), &DATA_PINS)
}
...
fn is_bs93(file_content: &[u8]) -> bool {
    file_content.len() > BS93_HEADER_LENGTH && &file_content[6..=9] == b"BS93"
}

fn is_lnx(file_content: &[u8]) -> bool {
    file_content.len() > LNX_HEADER_LENGTH && &file_content[0..=3] == b"LYNX"
}

fn is_nointro(file_content: &[u8]) -> bool {
    check_no_intro(file_content).is_ok()
}

impl Cartridge {
    /// Creates a new Cartridge instance from a slice of bytes.
    ///
    /// # Errors
    ///
    /// Returns an error if the cartridge file format cannot be identified.
    pub fn from_slice(data: &[u8]) -> Result<Self, &'static str> {
        let mut cart = Self::default();

        if is_lnx(data) {
            cart.lnx(data);
        } else if is_bs93(data) {
            cart.bs93(data);
        } else if is_nointro(data) {
            cart.nointro(data)?;
        } else {
    ...
    fn nointro(&mut self, file_content: &[u8]) -> Result<(), &'static str> {
        let l = file_content.len();
        let mut cart = if l <= _128K {
    ...
    pub fn block(&self) -> u16 {
        let pins = self.cart_pins();
        match &self.cart {
            CartType::Generic(c) => c.block(pins),
            CartType::None(..) => unreachable!("Trying to read to inexistant cart."),
        }
    }
    ...
    pub fn addr(&self) -> u16 {
        let pins = self.cart_pins();
        match &self.cart {
            CartType::Generic(c) => c.addr(pins),
            CartType::None(..) => unreachable!("Trying to read to inexistant cart."),
        }
    }
    ...
    pub fn data_address(&self) -> usize {
        let pins = self.cart_pins();
        match &self.cart {
            CartType::Generic(c) => c.data_address(pins),
            CartType::None(..) => unreachable!("Trying to read to inexistant cart."),
        }
    }
    ...
    fn lnx(&mut self, file_content: &[u8]) {
        ...
        match match bank_size {
            ...
            Err(e) => error!("{e:?}"),
            Ok(pins) => {
                let mut cart = CartridgeGeneric::new(u32::from(bank_size), pins);
    ...
    fn load_lnx_header(&mut self, file_content: &[u8]) {
        self.header
            .set_bank0_size(TO_U16!(file_content[4], file_content[5]));
        self.header
            .set_bank1_size(TO_U16!(file_content[6], file_content[7]));
        self.header
            .set_version(TO_U16!(file_content[8], file_content[9]));
        self.header
            .set_title(match String::from_utf8(file_content[10..=41].to_vec()) {
                Ok(t) => t,
                Err(_) => "Error".into(),
            });
        self.header
            .set_manufacturer(match String::from_utf8(file_content[42..=58].to_vec()) {
                Ok(m) => m,
                Err(_) => "Error".into(),
            });
    ...
    pub fn write_address_to_pins(&mut self, shifter: u8, ripple: u16, audin: u16) {
        ...
        pins = write_pins(pins, u16::from(shifter), &SHIFTER_PINS);
    ...
    fn cart_pins(&self) -> u32 {
        match &self.cart {
            ...
            CartType::None(..) => unreachable!("Trying to write to inexistant cart."),
        }
    }
                ...
                match buss {
                    BusStatus::PeekCart0 => {
                        if mikey_regs.data(SYSCTL1) & SYSCTL1_POWER != 0 {
                            ...
                            // info!("{:04X};{:04X};{:04X};{:02X}", self.block(), self.addr(), self.data_address(), data);
                            bus.set_data(data);
    ...
    pub fn copy_from(&mut self, other: &Cartridge) {
        ...
        match &other.cart {
            ...
            CartType::None(..) => unreachable!("Trying to read from an inexistant cart."),
        }
    }
}
