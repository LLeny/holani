...
impl AudioTimerRegisters {
    ...
    fn set_audio_feedback(&mut self, timer: &mut Timer, feedback: u16) {
        ...
        let mut ctrla = timer.control_a() & !0b1000_0000;
        ctrla |= (feedback as u8) & 0b1000_0000; // B7=feedback bit 7
        timer.set_control_a_transaprent(ctrla);
        self.feedback = (feedback as u8) & 0b0011_1111;
        self.feedback |= ((feedback & 0b0000_1100_0000_0000) >> 4) as u8; // B7= feedback bit 11, B6=feedback bit 10
    }
    ...
    pub fn set_audio_shift_register(&mut self, timer: &mut Timer, value: u16) {
        ...
        ctrlb &= !0b1111_0000; // B7=shift register bit 11, B6=shift register bit 10, B5=shift register bit 9, B4=shift register bit 8
        ctrlb |= ((value & 0b0000_1111_0000_0000) >> 4) as u8;
    ...
    pub fn audio_shift_register(&self, timer: &Timer) -> u16 {
        u16::from(self.shift_register) | ((u16::from(timer.control_b()) & 0b1111_0000) << 4)
    }

    #[inline]
    #[must_use]
    pub fn audio_feedback_taps(&self, timer: &Timer) -> u16 {
        let mut fb = u16::from(self.feedback) & 0b0011_1111;
        fb |= (u16::from(self.feedback) & 0b1100_0000) << 4;
        fb |= u16::from(timer.control_a() & 0b1000_0000);
        fb
    }
    ...
    pub fn done(&mut self, timer: &mut Timer) -> u8 {
        ...
        trace!(
            "AudioTimerRegisters output:0x{:02x} {:?}.",
            self.output() as u8,
            self
        );
        0
...
impl fmt::Debug for AudioTimerRegisters {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "AudioTimerRegisters volume:{}, feedback:{}, shift_register:{} output:{}, disabled:{}",
            self.volume, self.feedback, self.shift_register, self.output, self.disabled
        )
    }
}
